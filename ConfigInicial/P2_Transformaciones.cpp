...
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Proyecciones y transformaciones basicas, Cynthia B. Domínguez R. :D", nullptr, nullptr);
...
//	
	...
	/*
		...
		model = glm::scale(model, glm::vec3(8.0f, 1.0f, 3.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);


	//Para agregar tercer elemento
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-5.0f, 0.0f, 0.0f));
		model = glm::rotate(model, 0.5f, glm::vec3(1.5f, 0.0f, 0.0f)); // use to compare orthographic and perspective projection
